...
//
// ⏱️ Tick para actualizar ETA cada segundo
//
const tick = ref(0);
setInterval(() => {
  tick.value++;
...
const formatETA = (order) => {
  tick.value; // fuerza reactividad

  const estado = getEstadoOrden(order);

  if (estado !== "CON_PESAJE_INICIAL") return "—";

  const preset = getPreset(order);
  const masa = getMasa(order);
  const caudal = getCaudal(order);

  if (!preset || !masa || !caudal || caudal <= 0) return "—";

  const remaining = preset - masa;

  if (remaining <= 0) return "0m";

  const minutes = (remaining / caudal) * 60;
        ...
        >
          ...
          <div v-else class="overflow-x-auto">
            <table class="w-full text-sm">
              <thead
                class="border-b border-white/10 bg-black/30 text-gray-400"
              >
                <tr>
                  ...
                  <th class="px-5 py-4 text-left font-medium">ETA</th>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-5 py-4 text-gray-300 font-mono">
                    {{ formatETA(o) }}
                  </td>
